...
@if ($level == 'VL2')
    @if ($head->type == 'umum')
        @php
            $head = ($head->parent && $head->status == 5) ? $head->head[0] : $head;
                                        ...
                                        <div class="form-group d-flex justify-content-center">
                                            <div class="form-check d-inline-block">
                                                <input class="form-check-input" type="radio"
                                                    name="subDt[{{ $item->id }}][{{ $sub->id }}]"
                                                    value="1"
                                                    {{ old('subDt[' . $sub->id . ']') == '1' ? 'checked' : null }}>
                                                <label class="form-check-label">Ada</label>
                                            </div>
                                            <div class="form-check d-inline-block mx-3">
                                                @if (old('subDt[' . $sub->id . ']'))
                                                    <input class="form-check-input" type="radio"
                                                        name="subDt[{{ $item->id }}][{{ $sub->id }}]"
                                                        value="0"
                                                        {{ old('subDt[' . $sub->id . ']') == '0' ? 'checked' : null }}>
                                                @else
                                                    <input class="form-check-input" type="radio"
                                                        name="subDt[{{ $item->id }}][{{ $sub->id }}]"
                                                        value="0" {{ $head->status == 3 ? null : 'checked' }}>
                                                @endif
                                                <label class="form-check-label">Tidak Ada</label>
                                            </div>
                                            <div class="form-check d-inline-block">
                                                <input class="form-check-input" type="radio"
                                                    name="subDt[{{ $item->id }}][{{ $sub->id }}]"
                                                    value="2"
                                                    {{ old('subDt[' . $sub->id . '}]') == '2' ? 'checked' : null }}
...
@else
    @php
        ...
        $head = ($head->parent && !$old) ? $head->head[0] : $head;
        $old = $head->steps->where('kode', 'VL3')->first();
        if ($old) {            
            $da = json_decode($old->item);
